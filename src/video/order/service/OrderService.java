...
public class OrderService implements AppService {
        ...
        if(users.size() > 0) {
            ...
            if (userNums.contains(userNumber)) {
                ...
                System.out.printf("\n### [%s(%s) 회원님] 대여 처리가 완료되었습니다. 감사합니다!\n"
                        , rentalUser.getUserName(), rentalUser.getPhoneNumber().substring(phoneNumber.lastIndexOf("-") + 1)); // 하이픈은 빼기 위해
    ...
    private void returnProcess(int userNumber) {
        ...
        // 연체료 발생 여부를 확인하여 연체료가 존재한다면 추가로 얼마를 더 결제하라고 출력문을 띄워야 하고,
        ...
        if (orderList.containsKey(returnMovieNumber)) {
            ...
            returnMovie.setRentalUser(null); // 영화 객체에서 회원 정보 삭제
            returnMovie.setRental(false); // 영화 객체에서 대여 가능 여부 false(대여가능)으로 변경
